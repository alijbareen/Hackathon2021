...
class GameServer:
    ...
    def broadcast(self, host, port):
        ...
        if len(self.players) == 2:
            ...
            winner = ''
            self.timeForGame= time.time() + 10
            # Counting the scores and decide the Winner !
            players_keys = [name for name in self.players.keys()]
            while time.time() < self.timeForGame:

                if self.players[players_keys[0]]['answer'] != None:
                    if self.players[players_keys[0]]['answer'] == self.solution:
                        if self.players[players_keys[0]]['time'] < self.players[players_keys[1]]['time']:
                            winner = self.players[players_keys[0]]['teamName']
                            break
                    else:
                            winner = self.players[players_keys[1]]['teamName']
                            break

                if self.players[players_keys[1]]['answer'] != None:
                    if self.players[players_keys[1]]['answer'] == self.solution:
                        if self.players[players_keys[1]]['time'] < self.players[players_keys[0]]['time']:
                            winner = self.players[players_keys[1]]['teamName']
                            break
                    else:
                            winner = self.players[players_keys[0]]['teamName']
            ...
            for player in self.players:
                try:
                    if winner != '':
                        player.sendall((GameCloser % (self.solution, self.winner)).encode())
                    else:
                        player.sendall(("No one was answered").encode())
